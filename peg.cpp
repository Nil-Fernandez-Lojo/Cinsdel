...
/************************************************************************/
/*                                                                      */
/*        Modifications made by Nil Fernandez Lojo                      */
/*                                                                      */
/*        - added the option of adding the generator matrix to          */
/*          the output (-outputMode 2)                                  */
/*                                                                      */
/*        Bug reporting to: nf314@cam.ac.uk                             */
/************************************************************************/
#include <stdlib.h>
#include <stdio.h>
#include <string.h>
...
int main(int argc, char * argv[]){
  ...
  int       output_mode = 0; //by default only output parity check matrix and not generator
  ...
  USE:
    ...
    cout<<"          N, the 2nd line K, the number of message bits, the 3rd line M, the 4th lines     "<<endl;
    ...
    for(i=0;i<numArgs;i++){
      ...
      } else if(strcasecmp(argv[2*i+1], "-outputMode")==0) {
        output_mode=atoi(argv[2*i+2]);
      } else{
				goto USE;
      }
    }
...
//  /////////////////////// TEST END
  ...
  bigGirth	= new BigGirth(M, N, degSeq, degSeqChk, codeName, sglConcent, targetGirth);
	...
	// write code
  if (output_mode == 2) (*bigGirth).writeToFile();
  else (*bigGirth).writeToFile_Hcompressed();
